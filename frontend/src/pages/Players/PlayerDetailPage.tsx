...
import {
  FiArrowLeft, FiBarChart2, FiTarget, FiCalendar,
  FiMapPin, FiUsers, FiAward, FiTrendingUp,
  FiInfo, FiActivity
} from 'react-icons/fi';
...
const getPlayerRole = (battingStats: BattingStats[], bowlingStats: BowlingStats[]): string => {
  const totalRuns = battingStats.reduce((sum, s) => sum + safeNumber(s.runs), 0);
  const totalWickets = bowlingStats.reduce((sum, s) => sum + safeNumber(s.wickets), 0);
...
const calculateBattingMetrics = (stats: BattingStats[]) => {
  ...
  if (avgStrikeRate > 0) {
    ...
    accelerationScore = Math.min(98, Math.max(40, (avgStrikeRate - 50) * 1.4 + 40));
  }

  // Debug - check what average is being used
console.log('Batting Average for pressure calculation:', avgBattingAvg);
...
console.log('Final batting average used:', finalAvg);
...
if (finalAvg > 0) {
  ...
  return {
    ...
    overall: Math.round((consistencyScore + powerHittingScore + accelerationScore + pressureRating + longevityScore) / 5)
  };
};
// ============================================
...
const calculateBowlingMetrics = (stats: BowlingStats[]) => {
  ...
  if (wicketsPerMatch > 0) {
    wicketTakingScore = Math.min(98, Math.max(40, (wicketsPerMatch / 3) * 90 + 10));
  }
...
if (totalMatches > 0) {
  ...
  return {
    ...
    overall: Math.round((accuracyScore + wicketTakingScore + economyScore + strikeRateScore + matchWinningScore) / 5),
    totalWickets,
    totalFiveWickets
  };
};
      ...
      <table className="w-full text-sm border-collapse">
        ...
        <tbody>
          {sortedStats.map((stat) => (
            <tr key={stat.format} className="border-b hover:bg-gray-50 dark:hover:bg-gray-800/50">
              <td className="px-4 py-3 font-medium">{stat.format}</td>
              <td className="px-4 py-3 text-right">{safeNumber(stat.matches) || '-'}</td>
              <td className="px-4 py-3 text-right font-semibold text-green-600">
                {safeNumber(stat.runs).toLocaleString() || '-'}
              </td>
              <td className="px-4 py-3 text-right font-semibold">{safeNumber(stat.highest_score) || '-'}</td>
              <td className="px-4 py-3 text-right">{formatAverage(stat.batting_average)}</td>
              <td className="px-4 py-3 text-right">{formatStrikeRate(stat.strike_rate)}</td>
              <td className="px-4 py-3 text-right text-yellow-600 font-semibold">{safeNumber(stat.centuries)}</td>
              <td className="px-4 py-3 text-right">{safeNumber(stat.fifties)}</td>
        ...
        <tbody>
          {sortedStats.map((stat) => (
            <tr key={stat.format} className="border-b hover:bg-gray-50 dark:hover:bg-gray-800/50">
              ...
              <td className="px-4 py-3 text-right font-mono text-sm">{stat.best_figures || '-'}</td>
              <td className="px-4 py-3 text-right">{formatAverage(stat.average)}</td>
              <td className="px-4 py-3 text-right">{safeNumber(stat.economy)?.toFixed(2) || '-'}</td>
              <td className="px-4 py-3 text-right">{getBowlingStrikeRate(stat)}</td>
              <td className="px-4 py-3 text-right font-semibold text-purple-600">{safeNumber(stat.five_wickets) || 0}</td>
              <td className="px-4 py-3 text-right">{safeNumber(stat.runs_given)?.toLocaleString() || '-'}</td>
            </tr>
          ))}
        </tbody>
      </table>
...
// BIO CARD COMPONENT
// ============================================
const BioCard: React.FC<{ bio: PlayerBio; name: string; country: string }> = ({ bio, name, country }) => {
    ...
    <div className="bg-white dark:bg-gray-800 rounded-xl p-6 shadow border">
      <div className="flex flex-col md:flex-row gap-6">
        ...
        <div className="flex-1">
          <h1 className="text-2xl md:text-3xl font-bold">{bio.full_name || name}</h1>
          <div className="flex flex-wrap items-center gap-3 mt-2">
            ...
            {bio.birth_date && (
              <div className="flex items-center gap-1">
                ...
                <span className="text-sm">Born: {new Date(bio.birth_date).toLocaleDateString()}</span>
              </div>
            )}
            {bio.birth_place && (
              <div className="flex items-center gap-1">
    ...
    <div className="flex flex-wrap gap-2 max-h-32 overflow-y-auto">
      {teams.map((team, idx) => (
        <span key={idx} className="px-2 py-1 bg-gray-100 dark:bg-gray-700 rounded-full text-xs">
          ...
          {bio.description && (
            <div className="mt-4">
              <p className={`text-gray-600 dark:text-gray-400 text-sm leading-relaxed ${expanded ? '' : 'line-clamp-3'}`}>
                {bio.description}
              </p>
              {bio.description.length > 300 && (
                <button onClick={() => setExpanded(!expanded)} className="text-green-600 text-sm mt-1 hover:underline">
          ...
          {bio.awards && (
            <div className="mt-4 pt-3 border-t">
              <div className="flex items-center gap-2 text-sm">
                ...
                <span className="text-gray-600 dark:text-gray-400">{bio.awards}</span>
              </div>
            </div>
          )}
        </div>
...
export const PlayerDetailPage: React.FC = () => {
  ...
  if (!player) {
    return (
      <div className="text-center py-20">
        ...
        <Link to="/players" className="text-green-600 hover:underline">Back to Players</Link>
      </div>
    );
  }

  const totalRuns = player.batting_stats.reduce((sum, s) => sum + safeNumber(s.runs), 0);
  const totalWickets = player.bowling_stats.reduce((sum, s) => sum + safeNumber(s.wickets), 0);
  const totalCenturies = player.batting_stats.reduce((sum, s) => sum + safeNumber(s.centuries), 0);
  const totalFiveWickets = player.bowling_stats.reduce((sum, s) => sum + safeNumber(s.five_wickets), 0);
  ...
  { label: 'Consistency', value: battingMetrics.consistency, icon: '📊', color: 'text-blue-500' },
  { label: 'Power Hitting', value: battingMetrics.powerHitting, icon: '💥', color: 'text-red-500' },
  { label: 'Acceleration', value: battingMetrics.acceleration, icon: '⚡', color: 'text-yellow-500' },
  { label: 'Pressure Rating', value: battingMetrics.pressure, icon: '🎯', color: 'text-purple-500' },
  { label: 'Longevity', value: battingMetrics.longevity, icon: '📅', color: 'text-green-500' },
];

const bowlingMetricCards = [
  { label: 'Accuracy', value: bowlingMetrics.accuracy, icon: '🎯', color: 'text-blue-500' },
  { label: 'Wicket Taking', value: bowlingMetrics.wicketTaking, icon: '⚡', color: 'text-red-500' },
  { label: 'Economy', value: bowlingMetrics.economy, icon: '💰', color: 'text-green-500' },
  { label: 'Strike Rate', value: bowlingMetrics.strikeRate, icon: '🔄', color: 'text-purple-500' },
  { label: 'Match Winning', value: bowlingMetrics.matchWinning, icon: '🏆', color: 'text-yellow-500' },
];
...
const getPeakRankings = () => {
  ...
  return { bestRank: bestRank !== Infinity ? bestRank : null, bestFormat, allRanks };
};
...
const peakRankings = getPeakRankings();
  ...
  {playerRole === 'batsman' && (
    <div className="bg-gradient-to-br from-yellow-600 to-yellow-800 rounded-xl p-6 text-white text-center">
      <div className="text-2xl font-bold">
        {(() => {
          const sr = player.batting_stats.reduce((s, stat) => s + safeNumber(stat.strike_rate), 0) / (player.batting_stats.length || 1);
  ...
  {playerRole === 'bowler' && (
    <div className="bg-gradient-to-br from-yellow-600 to-yellow-800 rounded-xl p-6 text-white text-center">
      <div className="text-2xl font-bold">
        {(() => {
          const avg = player.bowling_stats.reduce((s, stat) => s + safeNumber(stat.average), 0) / (player.bowling_stats.length || 1);
  ...
  {playerRole === 'batsman' && (
    <div className="bg-gradient-to-br from-purple-600 to-purple-800 rounded-xl p-6 text-white text-center">
      ...
      const avg = totalInnings > 0 ? (totalRuns / totalInnings).toFixed(1) : '-';
      return avg;
    })()}
      </div>
  ...
  {playerRole === 'allrounder' && (
    <div className="bg-gradient-to-br from-purple-600 to-purple-800 rounded-xl p-6 text-white text-center">
      ...
      {playerRole === 'batsman' && (
        <MetricsCard title="Batting Performance Metrics" metrics={battingMetricCards} />
      )}
      {playerRole === 'bowler' && (
        <MetricsCard title="Bowling Performance Metrics" metrics={bowlingMetricCards} />
      )}
      {playerRole === 'allrounder' && (
        <>
          <MetricsCard title="Batting Performance Metrics" metrics={battingMetricCards} />
    ...
    {playerRole === 'batsman' && (
      <>
        <div className="bg-green-50 dark:bg-green-900/20 rounded-xl p-4 text-center hover:scale-105 transition-transform duration-200">
          <div className="text-xs text-gray-500 font-semibold">Best Score</div>
          <div className="text-xl font-bold text-green-700 dark:text-green-400">
            {Math.max(...player.batting_stats.map(s => safeNumber(s.highest_score)), 0)}*
          </div>
          <div className="text-[10px] text-gray-400 mt-1 font-medium">
            {player.batting_stats.find(s => safeNumber(s.highest_score) === Math.max(...player.batting_stats.map(s => safeNumber(s.highest_score))))?.format || 'ODI'}
          </div>
        </div>
        ...
        <div className="bg-yellow-50 dark:bg-yellow-900/20 rounded-xl p-4 text-center hover:scale-105 transition-transform duration-200">
          ...
          <div className="text-xl font-bold text-yellow-700 dark:text-yellow-400">{totalCenturies}</div>
          <div className="text-[10px] text-gray-400 mt-1 font-medium">Career</div>
        </div>
        ...
        <div className="bg-orange-50 dark:bg-orange-900/20 rounded-xl p-4 text-center hover:scale-105 transition-transform duration-200">
          ...
          <div className="text-xl font-bold text-orange-700 dark:text-orange-400">
            {player.batting_stats.reduce((s, stat) => s + safeNumber(stat.fifties), 0)}
          </div>
          <div className="text-[10px] text-gray-400 mt-1 font-medium">Career</div>
        </div>
...
<div className="bg-purple-50 dark:bg-purple-900/20 rounded-xl p-4 text-center hover:scale-105 transition-transform duration-200">
  <div className="text-xs text-gray-500 font-semibold mb-2">Peak Rank</div>
  
  {peakRankings.allRanks.length > 0 && 
   peakRankings.allRanks.every(r => r.rank === peakRankings.allRanks[0].rank) ? (
    <>
  ...
  {peakRankings.allRanks.length === 0 && (
    <>
      ...
      <div className="text-[10px] text-gray-400 mt-1">No ranking data</div>
    </>
  )}
</div>
      </>
    ...
    {playerRole === 'bowler' && (
      <>
        <div className="bg-blue-50 dark:bg-blue-900/20 rounded-xl p-4 text-center hover:scale-105 transition-transform duration-200">
          <div className="text-xs text-gray-500 font-semibold">Best Figure</div>
          <div className="text-xl font-bold text-purple-700 dark:text-purple-400">
            {(() => {
              const bestFigure = player.bowling_stats.map(s => s.best_figures).filter(Boolean)[0];
              if (!bestFigure) return 'N/A';
              const parsed = parseBestBowling(bestFigure);
              return parsed !== 'N/A' ? parsed : (player.bowling_stats.find(s => s.format === 'Test')?.best_figures?.replace(/-/g, '/') || 
                     player.bowling_stats.find(s => s.format === 'ODI')?.best_figures?.replace(/-/g, '/') || 'N/A');
            })()}
          </div>
          <div className="text-[10px] text-gray-400 mt-1 font-medium">
            {player.bowling_stats.find(s => s.best_figures && !s.best_figures.includes('Jul'))?.format || 'Career'}
          </div>
        </div>
        ...
        <div className="bg-red-50 dark:bg-red-900/20 rounded-xl p-4 text-center hover:scale-105 transition-transform duration-200">
          ...
          <div className="text-xl font-bold text-red-700 dark:text-red-400">{totalWickets}</div>
          <div className="text-[10px] text-gray-400 mt-1 font-medium">Career</div>
        </div>
        ...
        <div className="bg-green-50 dark:bg-green-900/20 rounded-xl p-4 text-center hover:scale-105 transition-transform duration-200">
          ...
          <div className="text-xl font-bold text-green-700 dark:text-green-400">
            {(() => {
              const econ = player.bowling_stats.reduce((s, stat) => s + safeNumber(stat.economy), 0) / (player.bowling_stats.length || 1);
        ...
        <div className="bg-purple-50 dark:bg-purple-900/20 rounded-xl p-4 text-center hover:scale-105 transition-transform duration-200">
          <div className="text-xs text-gray-500 font-semibold">3-Wk Hauls</div>
          <div className="text-xl font-bold text-purple-700 dark:text-purple-400">
    ...
    {playerRole === 'allrounder' && (
      <div className="col-span-5">
        <div className="grid grid-cols-6 gap-4">
          <div className="bg-green-50 dark:bg-green-900/20 rounded-xl p-4 text-center hover:scale-105 transition-transform duration-200">
            <div className="text-xs text-gray-500 font-semibold">Best Bat</div>
          ...
          <div className="bg-blue-50 dark:bg-blue-900/20 rounded-xl p-4 text-center hover:scale-105 transition-transform duration-200">
            <div className="text-xs text-gray-500 font-semibold">Best Bowl</div>
          ...
          <div className="bg-yellow-50 dark:bg-yellow-900/20 rounded-xl p-4 text-center hover:scale-105 transition-transform duration-200">
            <div className="text-xs text-gray-500 font-semibold">100s</div>
            <div className="text-lg font-bold text-yellow-700 dark:text-yellow-400">{totalCenturies}</div>
            <div className="text-[10px] text-gray-400 mt-1 font-medium">Career</div>
          </div>
...
<div className="bg-indigo-50 dark:bg-indigo-900/20 rounded-xl p-3 text-center">
  <div className="text-[10px] text-gray-500 font-semibold mb-1">Peak Rank</div>
        ...
        {activeTab === 'batting' && (
          <>
            <div className="flex items-center justify-between mb-4">
              ...
              <span className="text-sm text-gray-500">{player.batting_style || 'Right-hand bat'}</span>
            </div>
        ...
        {activeTab === 'bowling' && (
          <>
            <div className="flex items-center justify-between mb-4">
              ...
              <span className="text-sm text-gray-500">{player.bowling_style || 'Right-arm'}</span>
            </div>
