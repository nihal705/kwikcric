...
export const WorldCupHub: React.FC = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      const [tournamentsRes, rankingsRes] = await Promise.all([
        worldCupAPI.getAllTournaments('odi'),
        worldCupAPI.getTeamRankings('odi')
      ]);
  ...
  const tabs = [
    ...
    { id: 'history' as TabType, label: 'World Cup History' },
  ...
  <div className="bg-gradient-to-r from-green-700 to-green-900 py-5 md:py-7">
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <Link to="/world-cup" className="inline-flex items-center text-white/80 hover:text-white text-xs mb-2 md:mb-3">
        ← Back to ODI World Cup
      </Link>
    </div>
</div>
      {/* Hero Section - Compact */}
      <div className="relative bg-gradient-to-r from-green-700 to-green-900 py-8">
    ...
    />
          ...
          <h1 className="text-3xl md:text-4xl font-bold text-white mb-2">
            ICC Cricket Men's ODI World Cup
          </h1>
          <p className="text-sm text-green-100 max-w-2xl mx-auto">
            Complete history of cricket's biggest tournament - from 1975 to 2023
          </p>
        </div>
      </div>
        ...
        <AnimatePresence mode="wait">
          {activeTab === 'rankings' && (
            <motion.div
              ...
              exit={{ opacity: 0, y: -20 }}
            >
              <WorldCupTeamRankings rankings={teamRankings} />
            </motion.div>
          )}
          ...
          {activeTab === 'history' && (
            <motion.div
              ...
              exit={{ opacity: 0, y: -20 }}
            >
              <WorldCupHistory tournaments={tournaments} />
            </motion.div>
          )}
...
const WorldCupTeamRankings: React.FC<{ rankings: TeamRanking[] }> = ({ rankings }) => {
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {rankings.map((team) => (
          <Link key={team.team_name} to={`/world-cup/team/${encodeURIComponent(team.team_name)}`}>
            <motion.div
...
// World Cup History Component - Compact
const WorldCupHistory: React.FC<{ tournaments: Tournament[] }> = ({ tournaments }) => {
  ...
  return (
    <div className="space-y-3">
      <h2 className="text-xl font-bold text-center mb-4">All World Cup Editions</h2>
      <div className="grid grid-cols-1 gap-3">
        {tournaments.map((tournament, index) => (
          <Link key={tournament.year} to={`/world-cup/${tournament.year}`}>
            <motion.div
