...
const getTournamentDisplayName = (type: string): string => {
  switch (type) {
    case "t20":
      return "T20I World Cup";
    case "champions":
      return "Champions Trophy";
    case "test":
      return "ICC World Test Championship";
    default:
...
export const WorldCupDetailPage: React.FC = () => {
  ...
  const fetchDetails = async () => {
    ...
    try {
      ...
      const data = await worldCupAPI.getFullTournamentDetails(
        yearNum,
        tournamentType,
      );
      setDetails(data);
      ...
      setError(
        "Failed to load tournament details. Please check if backend is running.",
      );
    } finally {
      setLoading(false);
    }
  ...
  const getGradientColor = () => {
    switch (tournamentType) {
      ...
      case "test":
        return "from-teal-700 to-emerald-700";
      default:
  ...
  return (
    ...
    >
      {/* Header */}
      <div
        className={`relative bg-gradient-to-r ${getGradientColor()} py-8 md:py-12`}
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="text-center">
            ...
            <p className="text-xs md:text-sm text-white/80">
              Hosted by {tournament.host_country} • {tournament.total_teams}{" "}
              Teams • {tournament.total_matches} Matches
            </p>
          </div>
        </div>
      </div>
        ...
        <AnimatePresence mode="wait">
          ...
          {activeTab === "stats" && (
            ...
            >
              ...
              <TournamentStats
                tournamentId={details.tournament.id}
                year={details.tournament.year}
              />
            </motion.div>
          )}
...
const OverviewSection: React.FC<{
  ...
  return (
    <div className="space-y-5 md:space-y-6">
      {/* Winner Card */}
      <div
        className={`bg-gradient-to-r ${getWinnerGradient()} rounded-xl p-5 md:p-8 text-white text-center`}
      >
        ...
        <h2 className="text-2xl md:text-3xl font-bold mb-1 md:mb-2">
          {tournament.winner_name || "TBD"}
        </h2>
        <p className="text-base md:text-lg">
          {tournament.tournament_type?.toUpperCase() || "World Cup"}{" "}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3 md:gap-6">
        <div className="bg-white dark:bg-gray-800 rounded-lg md:rounded-xl p-3 md:p-6 shadow text-center border border-gray-200 dark:border-gray-700">
          <h3 className="text-xs md:text-sm font-semibold text-gray-500 mb-0.5 md:mb-1">
            Host Country
          </h3>
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 md:gap-4">
            ...
            {tournament.final_match_date && (
              <div>
                ...
                <div className="text-sm md:text-base font-semibold">
                  {new Date(tournament.final_match_date).toLocaleDateString()}
                </div>
              </div>
            )}
          </div>
...
const PointsTableSection: React.FC<{
  pointsTable: PointsTableEntry[];
  tournamentType: string;
}> = ({ pointsTable, tournamentType }) => {
  ...
  return (
    <div className="space-y-6 md:space-y-8">
      {Object.entries(groups).map(([groupName, entries]) => (
        <div
          key={groupName}
          className="bg-white dark:bg-gray-800 rounded-lg md:rounded-xl shadow overflow-hidden border border-gray-200 dark:border-gray-700"
        >
            ...
            <table className="w-full text-xs md:text-sm">
              ...
              <tbody>
                {entries.map((entry: PointsTableEntry, idx: number) => (
                  ...
                  >
                    <td className="px-3 md:px-4 py-2 md:py-3 font-medium text-xs md:text-sm">
                      {entry.team_name}
                    </td>
                    <td className="px-2 md:px-4 py-2 md:py-3 text-center">
                      {entry.matches_played}
                    </td>
                    <td className="px-2 md:px-4 py-2 md:py-3 text-center text-green-600 font-medium">
                      {entry.matches_won}
                    </td>
                    <td className="px-2 md:px-4 py-2 md:py-3 text-center text-red-600">
                      {entry.matches_lost}
                    </td>
                    <td className="px-2 md:px-4 py-2 md:py-3 text-center">
                      {entry.matches_tied || 0}
                    </td>
                    <td className="px-2 md:px-4 py-2 md:py-3 text-center">
                      {entry.matches_nr || 0}
                    </td>
                    ...
                    <td className="px-2 md:px-4 py-2 md:py-3 text-center font-mono">
...
const MatchesSection: React.FC<{
  ...
  const knockoutMatches = matches.filter(
    (match: Match) =>
      match.match_type === "semifinal" ||
      match.match_type === "final" ||
      match.match_type === "quarterfinal",
  );
  ...
  displayMatches = [...displayMatches].sort(
    (a, b) =>
      ...
      {(filter === "all" || filter === "knockout") &&
        knockoutMatches.length > 0 && (
          <div className="space-y-3 md:space-y-4">
            <h3 className="text-lg md:text-xl font-bold text-center text-gray-700 dark:text-gray-300">
              Knockout Stage
            </h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4">
              {knockoutMatches.map((match: Match) => (
                <MatchCard
                  ...
                  match={match}
                  isKnockout={true}
                  knockoutColor={getKnockoutColor()}
                />
              ))}
            </div>
          </div>
...
const MatchCard: React.FC<{
...
}> = ({ match, isKnockout, knockoutColor }) => {
  ...
  const getWinnerMessage = () => {
    if (isAbandoned) {
      ...
      if (match.abandonment_reason)
        return `Match abandoned due to ${match.abandonment_reason}`;
  ...
  return (
    <div
      className={`bg-white dark:bg-gray-800 rounded-lg p-3 md:p-4 shadow border ${
        isKnockout && knockoutColor
          ? knockoutColor
    ...
    >
      <div className="text-center mb-2 md:mb-3">
        ...
        {match.match_type === "final" && !isAbandoned && (
          <span className="ml-1 md:ml-2 text-xs bg-yellow-500 text-white px-1.5 py-0.5 rounded-full">
            FINAL
          </span>
        )}
        {match.match_type === "semifinal" && !isAbandoned && (
          <span className="ml-1 md:ml-2 text-xs bg-blue-500 text-white px-1.5 py-0.5 rounded-full">
            SF
          </span>
        )}
        {isAbandoned && (
          <span className="ml-1 md:ml-2 text-xs bg-red-500 text-white px-1.5 py-0.5 rounded-full">
            ABANDONED
          </span>
        )}
      </div>
      ...
      <div className="mt-2 md:mt-3 pt-1.5 md:pt-2 border-t border-gray-100 dark:border-gray-700 text-center">
        <span
          className={`text-xs md:text-sm font-semibold ${isAbandoned ? "text-orange-600" : "text-green-600"}`}
        >
          {getWinnerMessage()}
        </span>
