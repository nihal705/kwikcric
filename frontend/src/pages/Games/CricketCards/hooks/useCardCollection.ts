...
export const useCardCollection = () => {
  ...
  const updateOwnedCards = useCallback((allCards: Card[]) => {
    ...
    const completionRate =
      totalCardsInGame > 0
        ? Math.round((totalUniqueCards / totalCardsInGame) * 100)
        : 0;
    ...
    (card: Card) => {
      setCards((prevCards) => {
        ...
        if (existingCard) {
          ...
          newCards = [
            ...prevCards,
            { ...card, quantity: 1, isOwned: true, isFavorite: false },
          ];
        }

        localStorage.setItem(
          "cricket_cards_collection",
  ...
  const openPack = useCallback(
    async (packType: PackType): Promise<Card[]> => {
